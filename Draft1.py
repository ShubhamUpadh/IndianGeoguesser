...
def fxCheckAns(val):
    if val.lower().strip() == state_n.lower():
        return True
    return False
...
label1 = Label(root, text="This is your attempt #" + str(counter), font=("Arial", 12))
...
def fx_NameCheck1():
    ...
    if not e.get().lower() in states_lower:
        ...
        slabel = Label(root, text="Your input is invalid.Please RECHECK your input",
                       font=("Arial", 15))
        slabel.pack()
    if e.get().lower().strip() == state_n.lower():
        slabel = Label(root, text="Correct Input, The name of the state is " + state_n.upper() + " !!", font=("Arial", 15))
        slabel.pack()

    else:
        if flag:
            klabel = Label(root, text=" Incorrect. The name of the state is not " + e.get().upper() + "! Please try "
                                                                                                      "again.",
                       font=("Arial", 15))
            klabel.pack()
